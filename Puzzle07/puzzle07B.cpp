...
struct hand_t {
    ...
    void classify() {
        ...
        // all wild cards -- pick the best hand possible (5x A)
        if (wild_count == 5) {
            sorted_cards.push_back({'A', 5});
        // otherwise, use wild count to duplicate best card in hand
        } else{
            ...
            // add wild count to best card in hand
            // if 1 wild, can turn 4-kind into 5-kind
            //                     3-kind into 4-kind
            //                     2-pair into full house
            //                     1-pair into 3-kind
            //                     high card into 1-pair
            // if 2 wild, can turn full house (XXXJJ) into 5-kind
            //                     2-pair (XXYJJ) into 4-kind
            //                     1-pair (XYZJJ) into 3-kind
            // if 3 wild, can turn full house (XXJJJ) into 5-kind
            //                     3-kind (XYJJJ) into 4-kind
            // if 4 wild, can turn 4-kind (XJJJJ) into 5-kind
            // if 5 wild, can turn 5-kind (JJJJJ) into best 5-kind (AAAAA)
            sorted_cards.begin()->second += wild_count;
        }
